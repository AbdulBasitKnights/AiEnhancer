...
import com.aiface.aging.utils.GeometryUtils;
import com.aiface.aging.utils.ImageDecoder;
import com.aiface.aging.utils.MultiTouchHandler;
import com.aiface.aging.utils.ResultContainer;

import java.util.ArrayList;
import java.util.List;


public class GridPhotoImageView extends AppCompatImageView {
    private static final String TAG = GridPhotoImageView.class.getSimpleName();

    public interface OnImageClickListener {
        void onLongClickImage(GridPhotoImageView view);

        void onDoubleClickImage(GridPhotoImageView view);

        void onSingleClickImage(GridPhotoImageView view);

        void onTouchImage(GridPhotoImageView view);
    }
    private Paint mBorderPaint;
    private boolean mHighlightEnabled = false;
    private static GridPhotoImageView currentHighlightedView = null;
    ...
    private void initPaints() {
        int color = Color.parseColor("#8850FF");
        mBorderPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mBorderPaint.setColor(color);
        mBorderPaint.setStyle(Paint.Style.STROKE);
        mBorderPaint.setStrokeWidth(8); // Border thickness
    }
    public GridPhotoImageView(final Context context, PhotoItem photoItem) {
        super(context);
        mPhotoItem = photoItem;
        initPaints();
//        int padding = (int) (mBorderPaint.getStrokeWidth() / 2);
...
//                    FirebaseCrash.report(err);
                    if (context instanceof Activity) {
                        ((Activity) context).runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                try {
                                    Toast.makeText(getContext().getApplicationContext(), "Out of Memory", Toast.LENGTH_SHORT).show();
                                } catch (Exception ex) {
                                    ex.printStackTrace();
                                }
                            }
        ...
        mGestureDetector = new GestureDetector(getContext(), new GestureDetector.SimpleOnGestureListener() {
            ...
            public void onLongPress(MotionEvent e) {
                if (mOnImageClickListener != null) {
                    mOnImageClickListener.onLongClickImage(GridPhotoImageView.this);
                }
            }
            ...
            public boolean onDoubleTap(MotionEvent e) {
                if (mOnImageClickListener != null) {
                    mOnImageClickListener.onDoubleClickImage(GridPhotoImageView.this);
                }
                return true;
            }
            ...
            public boolean onSingleTapUp(MotionEvent e) {
                if (mOnImageClickListener != null) {
                    mOnImageClickListener.onSingleClickImage(GridPhotoImageView.this);
                }
                return true;
            }
        });
    ...
    public void swapImage(GridPhotoImageView view) {
...
//        recycleImage();
    ...
    public void toggleHighlight() {
        if (mHighlightEnabled) {
            ...
            if (currentHighlightedView != null && currentHighlightedView != this) {
                currentHighlightedView.unhighlight();
            }
            // Highlight this view
            mHighlightEnabled = true;
            invalidate();
            currentHighlightedView = this;
        }
    }
        ...
        if (!mEnableTouch) {
            ...
            if (mSelected) {
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    mSelected = false;
                    if (mOnImageClickListener != null) {
                        mOnImageClickListener.onTouchImage(GridPhotoImageView.this);
                    }
                }
    ...
    protected void onDetachedFromWindow() {
        ...
        if (currentHighlightedView == this) {
