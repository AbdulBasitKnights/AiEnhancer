...
public class PuzzleView extends FrameLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        handleBarPaint.setStrokeWidth(lineSize * 4);
    ...
    private void drawSelectedAreaOld(Canvas canvas, PuzzlePiece piece) {
    ...
    private void drawSelectedArea(Canvas canvas, PuzzlePiece piece) {
        ...
        // define the scale factor for reducing handle bar length
        float scaleFactor = 0.60f; // Reduce the length to 60%

        // draw handle bar
        for (Line line : area.getLines()) {
            if (puzzleLayout.getLines().contains(line)) {
                PointF[] handleBarPoints = area.getHandleBarPoints(line);
                // Calculate midpoint of the handle bar
                float midX = (handleBarPoints[0].x + handleBarPoints[1].x) / 2;
                float midY = (handleBarPoints[0].y + handleBarPoints[1].y) / 2;

                // Adjust handle bar points to be closer to the midpoint
                handleBarPoints[0].x = midX + (handleBarPoints[0].x - midX) * scaleFactor;
                handleBarPoints[0].y = midY + (handleBarPoints[0].y - midY) * scaleFactor;
                handleBarPoints[1].x = midX + (handleBarPoints[1].x - midX) * scaleFactor;
                handleBarPoints[1].y = midY + (handleBarPoints[1].y - midY) * scaleFactor;

                // Draw the modified handle bar
                canvas.drawLine(handleBarPoints[0].x, handleBarPoints[0].y, handleBarPoints[1].x,
                        handleBarPoints[1].y, handleBarPaint);
                canvas.drawCircle(handleBarPoints[0].x, handleBarPoints[0].y, lineSize * 3 / 2,
                        handleBarPaint);
    ...
    private void moveLine(Line line, MotionEvent event) {
        ...
        if (needUpdate) {
            ...
            // Add null check for moveListener
            if (moveListener != null) {
                moveListener.isMoved();
            }
        }
    }
    ...
    public List<Drawable> getPuzzleDrawableList() {
        final int size = puzzlePieces.size();
        final List<Drawable> drawable = new ArrayList<>(size);
        puzzleLayout.sortAreas();
        for (int i = 0; i < size; i++) {
            Area area = puzzleLayout.getArea(i);
            PuzzlePiece piece = areaPieceMap.get(area);
            drawable.add(piece.getDrawable());
        }
        return drawable;
    }
    ...
    public void setMoveListener(MoveListener moveListener1) {
        if (moveListener1 == null) {
            moveListener = () -> {
                // Default no-op implementation
            };
        } else {
            moveListener = moveListener1;
        }
    }
