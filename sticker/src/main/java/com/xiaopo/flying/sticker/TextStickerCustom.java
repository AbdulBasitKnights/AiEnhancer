...
public class TextStickerCustom extends Sticker {
    ...
    public TextStickerCustom(@NonNull Context context) {
        this(context, null);
    }

    public TextStickerCustom(@NonNull Context context, @Nullable Drawable drawable) {
        ...
        maxTextSizePixels = convertSpToPx(20);
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        int boundsHeight = staticLayout.getHeight();
        int canvasHeight = canvas.getHeight();
        int drawableHeight = (int) (canvasHeight * Math.max(0.10f,
                1.15F * boundsHeight / canvasHeight));
        height = drawableHeight;
        Matrix matrix = getMatrix();
        canvas.save();
        canvas.concat(matrix);
        if (drawable != null) {
            realBounds = new Rect(0, 0, getWidth(), drawableHeight);
            textRect = new Rect(0, 0, getWidth(), drawableHeight);
        ...
        canvas.save();
        canvas.concat(matrix);
    /*if (boundsHeight < drawableHeight) {
      ...
      float textYCoordinate = (drawableHeight - boundsHeight) / 2;
    ...
    @NonNull
    @Override
    public TextStickerCustom setAlpha(@IntRange(from = 0, to = 255) int alpha) {
    ...
    @NonNull
    public TextStickerCustom setDrawable(@NonNull Drawable drawable, @Nullable Rect region) {
        this.drawable = drawable;
        realBounds.set(0, 0, getWidth(), getHeight());
        if (region == null) {
            textRect.set(0, 0, getWidth(), getHeight());
    ...
    @NonNull
    public TextStickerCustom setTextColor(@ColorInt int color) {
    ...
    @NonNull
    public TextStickerCustom setMaxTextSize(@Dimension(unit = Dimension.SP) float size) {
     ...
     */
    @NonNull
    public TextStickerCustom setMinTextSize(float minTextSizeScaledPixels) {
        minTextSizePixels = convertSpToPx(minTextSizeScaledPixels);
        return this;
    }

    @NonNull
    public TextStickerCustom setLineSpacing(float add, float multiplier) {
    ...
    @NonNull
    public TextStickerCustom setText(@Nullable String text) {
        this.text = text;
        return this;
    }
    ...
    @Override
    public void setTextGradient(String[] colors) {
        ...
        Shader textShader = new LinearGradient(0, 0, getWidth(), getMinTextSizePixels(),
                colorList, null, Shader.TileMode.REPEAT);
        textPaint.setShader(textShader);
    }

    @Override
    public void setNeonText(int color, BlurMaskFilter.Blur style) {
        BlurMaskFilter filter = new BlurMaskFilter(15, style);
