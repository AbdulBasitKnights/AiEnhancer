...
public class StickerView extends FrameLayout {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            // Check if the touch event is inside any stickers or icons
            boolean isInsideSticker = false;
            for (Sticker sticker : stickers) {
                if (isInStickerArea(sticker, x, y)) {
                    isInsideSticker = true;
                    break;
                }
            }
            ...
            // If the touch event is outside any stickers and icons, hide the borders
            if (!isInsideSticker) {
                hideBorders();
            }
        }
    ...
    private boolean isInIconArea(@NonNull BitmapStickerIcon icon, float x, float y) {
        float iconX = icon.getX();
        float iconY = icon.getY();
        float iconRadius = icon.getIconRadius();
        return Math.pow(x - iconX, 2) + Math.pow(y - iconY, 2) <= Math.pow(iconRadius, 2);
    }
